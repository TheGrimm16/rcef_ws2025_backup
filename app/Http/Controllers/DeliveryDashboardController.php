...
//STOPPED HERE
	...
	public function extractDeletedIARs()
    {
        ...
        // dd($date);
        $data = DB::table($prefix.'rcep_delivery_inspection.tbl_delivery_transaction')
        ->where('status', 3)
        ->get();

        foreach($data as $row) {
            $prv = substr($row->prv_dropoff_id, 0, 6);
            $getProvMun = DB::table($prefix.'rcep_delivery_inspection.lib_prv')
            ->where('prv','LIKE',$prv)
            ->first();

            $getCoopName = DB::table($prefix.'rcep_seed_cooperatives.tbl_cooperatives')
            ->where('accreditation_no','LIKE',$row->accreditation_no)
            ->first();

            $getUser = DB::table($prefix.'sdms_db_dev.users')
            ->where('userId',$row->user_id)
            ->first();

            $IAR = '';
            $getIAR = DB::table($prefix.'rcep_delivery_inspection.iar_print_logs')
            ->where('batchTicketNumber','LIKE',$row->batchTicketNumber)
            ->first();
            ...
            $excel_data[] = [
                ...
                'Province Name' => $getProvMun->province,
                'Municipality Name' => $getProvMun->municipality,
                'DOP ID' => $row->prv_dropoff_id,
                'SGC/A Name' => $getCoopName->coopName,
                'MOA Number' => $row->moa_number,
                'Accreditation Number' => $row->accreditation_no,
                'Delivery Date' => $row->delivery_date,
                'Date Created' => $row->date_created,
                'Status' => $row->status,
                'IAR Number' => $IAR,
                'Batch Ticket Number' => $row->batchTicketNumber,
                'Created By' => $getUser->username,
                'Buffer Delivery (1=true, 0=false)' => $row->isBuffer,
                'Date Synced' => $row->sync_date,
                'Replacement Seeds (1=true, 0=false)' => $row->is_for_replacement,
                'Seed Distribution Mode' => $row->seed_distribution_mode
            ];

        }

        $filename = 'Deleted IARs_as_of_'.$date;
        return Excel::create($filename, function($excel) use ($excel_data) {
            $excel->sheet("Deleted IARs", function($sheet) use ($excel_data) {
                $sheet->fromArray($excel_data);
                $sheet->getStyle('A1:Q1')->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('00B53F');
                $border_style = array(
                    'borders' => array(
                        'allborders' => array(

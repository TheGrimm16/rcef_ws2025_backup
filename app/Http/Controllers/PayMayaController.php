...
class PayMayaController extends Controller
{
            ...
            public function ebinhiCss(){
                $quest = DB::table($GLOBALS['season_prefix']."rcep_paymaya.lib_survey_questions")
                    ->select("rank as question_number","question")
                    ->orderBy("rank")
                    ->get();
                    $quest = json_decode(json_encode($quest), true); 
         
                $returns = DB::table($GLOBALS['season_prefix']."rcep_css.updated_bep_responses")
                ->select("claim_code", "name", "b_id_1", "b_id_2", "b_id_3", "b_id_4", "b_id_5", "b_id_6", "b_id_7", "b_id_8", "b_id_9", "b_id_10", "b_id_11", "b_id_12", "b_id_13", "b_id_14", "b_id_15", "b_id_16", "b_id_17")
                    ->join($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries","tbl_beneficiaries.paymaya_code","=","updated_bep_responses.claim_code")    
                
                ->groupBy("claim_code")
                    ->orderBy("encoded_on", "DESC")
                    ->get();
                
                    $excel_data = array();
                foreach($returns as $ins){

                    $prv = substr($ins->claim_code,0,4);
                    // dd($prv );
                    $bday = DB::table($GLOBALS['season_prefix']."prv_".$prv.".farmer_information_final")
                        ->where("rcef_id", $ins->claim_code)
                        ->value("birthdate");
                    ...
                    // $excel_data = json_decode(json_encode($returns), true); //convert collection to associative array to be converted to excel
                    return Excel::create("Binhi e-Padala Survey Returns"."_".date("Y-m-d g:i A"), function($excel) use ($excel_data,$quest) {
                        $excel->sheet("Returns", function($sheet) use ($excel_data) {
                            $sheet->fromArray($excel_data);
                            $sheet->freezeFirstRow();
                        });
                        
                        $excel->sheet("Question List", function($sheet) use ($quest) {
                            $sheet->fromArray($quest);
                            $sheet->freezeFirstRow();
                        });
            ...
            public function check_code_survey(){
              ...
              $api_key = $request['api_key'];
                $fullName =substr($claim_code,1,128);
                $rsbsa_control_no = substr($claim_code,1,128);
               ...
               if($api_key == "leKvg/BuUk5SeGTQEw/D1BZMKRaC3/BpnShD5UQ92gE="){
                    ...
                    ->orwhere("rsbsa_control_no", "LIKE", "%".$rsbsa_control_no."%")
                    
                    ->first();
                    if(count($claim)>0){

                    $survey = DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_survey_answer")
                        ->where("claim_code", $claim->paymaya_code)
                        ->get();
                    if(count($survey)>0){
                        $beneficiary = DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries")
                            ...
                            ->where("tbl_beneficiaries.paymaya_code", $claim->paymaya_code)
                            ->limit(1)
                            ->get();
                    }else{
                        $beneficiary = DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries")
           ...
           //EXCEL REPORT
           public function export_ebinhi_no_claim($tag,$province, $municipality){
            ...
            if($tag == "sched"){
                $claim_data =  DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_claim")
                    ...
                    $claim_data = json_decode(json_encode($claim_data),true);

                $excel_data = DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries")
                    ->where("province", $province)
                    ->where("municipality", $municipality)
                    ->whereNotIn("paymaya_code",$claim_data)
                    ->get();


                $excel_data = json_decode(json_encode($excel_data), true); //convert collection to associative array to be converted to excel
                return Excel::create("EBINHI_SCHEDULED_NO_CLAIM".$province."_".$municipality."_".date("Y-m-d g:i A"), function($excel) use ($excel_data) {
            ...
            }elseif($tag=="not_sched"){
                $excel_data = DB::table($GLOBALS['season_prefix']."rcep_paymaya.sed_verified")
                ...
                $excel_data = json_decode(json_encode($excel_data), true); //convert collection to associative array to be converted to excel
                return Excel::create("EBINHI_NOT_SCHEDULED".$province."_".$municipality."_".date("Y-m-d g:i A"), function($excel) use ($excel_data) {
                    $excel->sheet("BENEFICIARY LIST", function($sheet) use ($excel_data) {
                        $sheet->fromArray($excel_data);
                        $sheet->freezeFirstRow();
        ...
        public function exportUnclaimedCodes($province,$municipality,$mode,$sched_start,$sched_end){
            ...
            if($mode =="download"){
                ...
                if($municipality =="-")$municipality="%";
                     ...
                     $raw = "SELECT * from ".$GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries where province LIKE '%".$province."%' and municipality LIKE '%".$municipality."%' 
                        and date_format(str_to_date(schedule_start, '%Y-%m-%d'), '%Y-%m-%d') between '".$sched_start."'
                        and  '".$sched_end."'
                      and paymaya_code not in (SELECT paymaya_code from ".$GLOBALS['season_prefix']."rcep_paymaya.tbl_claim) ";
                ...
                $data = DB::select(DB::raw($raw));
                    $success = array();
                    $failed = array();
                    foreach($data as $unclaimed_data){
                    
                        $prv_code = DB::table($GLOBALS['season_prefix']."rcep_delivery_inspection.lib_prv")
                            ->where("province", $unclaimed_data->province)
                            ->first();

                            if($prv_code != null){
                                $farmer_info = DB::table($GLOBALS['season_prefix']."prv_".$prv_code->prv_code.".farmer_information_final")
                                    ->where("rcef_id", $unclaimed_data->paymaya_code)
                                    ->where("firstName", $unclaimed_data->firstname)
                                    ->where("lastName", $unclaimed_data->lastname)
                                    ->first();

                                    if($farmer_info != null){
                                        $release = count(DB::table($GLOBALS['season_prefix']."prv_".$prv_code->prv_code.".new_released")
                                                ->where("rcef_id", $farmer_info->rcef_id)
                                                ->get());
                                        if($release > 0){
                                            array_push($failed,array(
                                                "rcef_id"=> $farmer_info->rcef_id,
                                                "lastname"=> $unclaimed_data->lastname,
                                                "firstname"=> $unclaimed_data->firstname,
                                                "reason" => "Already has claim"
                                            ));
                                        }else{
                                            ...
                                            $current_claimable = $farmer_info->final_claimable - $farmer_info->total_claimed;

                                            if($current_claimable <= $check_total_claimed){
                                                array_push($failed,array(
                                                    "rcef_id"=> $farmer_info->rcef_id,
                                                    "lastname"=> $unclaimed_data->lastname,
                                                    "firstname"=> $unclaimed_data->firstname,
                                                    "reason" => "Claimable bags exhausted"
                                                ));
                                            }else{



                                                DB::table($GLOBALS['season_prefix']."prv_".$prv_code->prv_code.".farmer_information_final")
                                                ->where("id", $farmer_info->id)
                                                ->update([
                                                    "is_ebinhi" => 0,
                                                    "total_claimed" => $check_total_claimed + $farmer_info->total_claimed
                                                ]);
                                        ...
                                        $farmer_info = DB::table($GLOBALS['season_prefix']."prv_".$prv_code->prv_code.".farmer_information_final")
                                        ->where("rsbsa_control_no", $unclaimed_data->rsbsa_control_no)
                                        ->where("firstName", $unclaimed_data->firstname)
                                        ->where("lastName", $unclaimed_data->lastname)
                                        ->where("midName", $unclaimed_data->middname)
                                        ->get();
                                        if(count($farmer_info)==1){
                                            $farmer_info = $farmer_info[0];
                                            ...
                                            if($release > 0){
                                                ...
                                                $check_total_claimed = count(DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_claim")
                                                ->where("paymaya_code", $unclaimed_data->paymaya_code)
                                                ->get());
                                            ...
                                            array_push($failed,array(
                                                "rcef_id"=> $unclaimed_data->paymaya_code,
                                                "lastname"=> $unclaimed_data->lastname,
                                                "firstname"=> $unclaimed_data->firstname,
                                                "reason" => "Cannot Find on Conventional list"
                                            ));
                    ...
                    return Excel::create("eBinhi_transfer_report".$municipality."_".date("Y-m-d g:i A"), function($excel) use ($failed, $success) {
                        $excel->sheet("SUCCESS", function($sheet) use ($success) {
                            $sheet->fromArray($success);
                            $sheet->freezeFirstRow();
                        });
                       
                        $excel->sheet("FAILED", function($sheet) use ($failed) {
                            $sheet->fromArray($failed);
                            $sheet->freezeFirstRow();
                        });
        ...
        public function paymaya_cancel_delivery(Request $request){
         
                $data = DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_claim")
                    ->where("paymaya_code", $request->code)
                    ->where("is_paid", 0)
                    ->orderBy("date_created", "DESC")
                    ->first();

                    if($data != null){
                        $date_release = $data->date_created;
                               ...
                               $claim_data =  DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_claim")
                                ...
                                ->get();
                                    ...
                                    foreach($claim_data as $claim){
                                        ...
                                        DB::table($GLOBALS['season_prefix']."rcep_paymaya.cancelled_claim")
                                            ->insert($claim);
                                ...
                                return json_encode("Cannot Cancel");
                            }
                    }else{
                        return json_encode("Cannot Cancel");
                    }

                    DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_cancel_distribution_logs")
                    ->insert([
                        "paymaya_code" => $request->code,
        ...
        public function genTable_with_code(Request $request){
            ...
            $list = DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries")
                ->select($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries.*", $GLOBALS['season_prefix']."rcep_paymaya.lib_ebinhi_dop.coop_name as coopName")
        ...
        foreach($list as $row){
            ...
            if(Auth::user()->roles->first()->name == "rcef-programmer"){
                ...
                //             if($date_release == $date_now){
                //                }else{
                //                 $btn = "<button id='cancel_button' onclick='cancel_distri(".'"'.$row->paymaya_code.'"'.")' class='btn btn-dark' disabled>Cancel Distribution </button>";
                //             }
                //     }else{
                //         $btn = "<button id='cancel_button' onclick='cancel_distri(".'"'.$row->paymaya_code.'"'.")' class='btn btn-dark' disabled>Cancel Distribution </button>";
                //     }
                    ...
                    $btn = "<button id='cancel_button' onclick='cancel_distri(".'"'.$row->paymaya_code.'"'.")' class='btn btn-danger' >Cancel Distribution </button>";
    ...
    public function generate_municipal_report($province, $municipality){
        ...
        try {
            ...
            $provincial_data = DB::table($GLOBALS['season_prefix'].'rcep_paymaya.tbl_claim')
                ->where('province', $province)
                ->where('municipality', $municipality)
                // ->whereBetween("date_created", ['2023-04-01', '2023-04-30'])
            ...
            foreach($provincial_data as $row){
             // dd($row);
                ...
                if(count($beneficiary_details)==null){
                   // dd($row);
                ...
                $row_data["sex"] = $beneficiary_details->sex;                
...
// "qr_code" => $row->qr_code, "seedTag" => $row->seedTag, "seedVariety" => $row->seedVariety,
            ...
            return Excel::create("PAYMAYA_"."$province"."_".$municipality."_".date("Y-m-d g:i A"), function($excel) use ($excel_data, $var_count) {
                $excel->sheet("BENEFICIARY LIST", function($sheet) use ($excel_data, $var_count) {
                    $sheet->fromArray($excel_data, null, 'A1', false, false);
                    $title_arr = array(
                        "rsbsa_control_no","firstname","middname","lastname","extname","sex","paymaya_code","date_created","province","municipality","barangay","drop_off_point","phoneNumber","area","seed_grower"
                    );
    ...
    public function beneficiary_report_home(){
        $provincial_data = DB::table($GLOBALS['season_prefix'].'rcep_paymaya.paymaya_basis')
            ->select('paymaya_basis.province', DB::raw('SUM(beg_beneficiaries) as beg_beneficiaries'),
        ...
        $province_arr = array();
        foreach($provincial_data as $row){
            $total_bags_view = DB::table($GLOBALS['season_prefix']."rcep_paymaya.tbl_beneficiaries")
            ...
            $claim_area =  DB::table($GLOBALS['season_prefix']."rcep_paymaya.paymaya_claim_area")->where('province', $row->province)->sum("sum_area");
          ...
          $bene =   DB::table($GLOBALS["season_prefix"]."rcep_paymaya.paymaya_basis")
                ->where("province", $row->province)
                ->sum("beg_beneficiaries");

            array_push($province_arr, array(
                "province" => $row->province,
                "total_beneficiaries" => number_format($bene),
                "total_area" => number_format($row->beg_total_area,2,".",","),
                "total_bags" => number_format($total_bags_view),
                ...
                "claim_total_area" => number_format($claim_area,2),
                "accepted_bags" => number_format($ebinhi),
            ));
       ...
       public function beneficiary_report_provincial_tbl(Request $request){
        ...
        $municipal_data = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_actual_delivery')
            ->select('province', 'municipality')
            ->where('province', $request->province)
            ->where("qrValStart", "!=", "")
            ->groupBy('municipality')
        ...
        foreach($municipal_data as $row){
            ...
            $claim_area =  DB::table($GLOBALS['season_prefix']."rcep_paymaya.paymaya_claim_area")->where('province', $row->province)
            ->where("municipality", $row->municipality)
            ->sum("sum_area");
...
/*                 ->where('province', $row->province)
            ...
            $claim_total_beneficiaries = count($claim_total_beneficiaries);
            ...
            array_push($municipal_arr, array(
               ...
               "claim_total_beneficiaries" => number_format($claim_total_beneficiaries),
                "claim_total_bags" => number_format($claim_total_bags),
                // "claim_total_area" => number_format($claim_total_area,2,".",","),
                "claim_total_area" => $claim_area,
                "accepted_bags" => $ebinhi,
                
            ));
        }
       // dd($municipal_arr);claim_total_beneficiaries

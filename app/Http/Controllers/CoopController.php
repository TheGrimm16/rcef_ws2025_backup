...
{
    public function generateCoopQR($coop_name, $accreditation_no){
        echo '<br> <br><center> <img class="qr_class" src="data:image/jpg;base64, '.base64_encode(QrCode::format('png')->size(240)->margin(0)->generate(str_replace("*","/",$accreditation_no))) .'" style="height:180px;width:180px;margin-right:0px;margin-top:5px;margin-bottom:5px;"/> <br>
        <strong>'.str_replace("*","/",$coop_name).'</strong> <br>
        <strong>'.str_replace("*","/",$accreditation_no).'</strong>

        </center>';
    }


    public function adjustment_gentable(Request $request){
        $data = DB::connection("seed_coop_db")->table("tbl_adjustment_logs")
            ->select("tbl_adjustment_logs.*", "tbl_cooperatives.coopName")
            ->join("tbl_cooperatives", "tbl_cooperatives.coopId", "=", "tbl_adjustment_logs.coopId")
            ->where("tbl_adjustment_logs.coopId", $request->coop_id)
            ->where(DB::raw("DATE_FORMAT(tbl_adjustment_logs.date_created, '%Y-%m-%d')"), $request->logs_date)
            ->get();
        $data = collect($data);
        ...
        ->addColumn('region', function($row){
            if($row->region_from != "NEW"){
                if($row->region_from != $row->region_to){
                    $result = '<span class="badge badge-info" style="font-size:15px;">'.$row->region_from.'</span>'." >> ".'<span class="badge badge-success" style="font-size:15px;">'.$row->region_to.'</span>';
                }else{
                    $result ='<span class="badge badge-success" style="font-size:15px;">'.$row->region_to.'</span>';
                }
            }else{
                $result ='<span class="badge badge-success" style="font-size:15px;">'.$row->region_to.'</span>';
            }
        ...
        ->addColumn('seed_variety', function($row){
            if($row->seedvariety_from != "NEW"){
                if($row->seedvariety_from != $row->seedvariety_to){
                    $result = '<span class="badge badge-info" style="font-size:15px;">'.$row->seedvariety_from.'</span>'." >> ".'<span class="badge badge-success" style="font-size:15px;">'.$row->seedvariety_to.'</span>';
                }else{
                    $result ='<span class="badge badge-success" style="font-size:15px;">'.$row->seedvariety_to.'</span>';
                }
            }else{
                $result ='<span class="badge badge-success" style="font-size:15px;">'.$row->seedvariety_to.'</span>';
            }
        ...
        ->addColumn('volume', function($row){
            ...
            if($row->volume_from != $row->volume_to){
                $result = '<span class="badge badge-info" style="font-size:15px;">'.number_format($row->volume_from).'</span>'." >> ".'<span class="badge badge-success" style="font-size:15px;">'.number_format($row->volume_to).'</span>';
            }else{
                $result ='<span class="badge badge-success" style="font-size:15px;">'.number_format($row->volume_to).'</span>';
            }
            return $result;
    ...
    public function adjusment_dates(Request $request){
        $dates =  DB::connection("seed_coop_db")->table("tbl_adjustment_logs")
                ->select(DB::raw("DATE_FORMAT(date_created, '%Y-%m-%d') as date_created"),DB::raw("DATE_FORMAT(date_created, '%M %d, %Y') as date_name"))
    ...
    public function commitment_logs_ui(){
        
        $coop_count = count(DB::connection("seed_coop_db")->table("tbl_cooperatives")
            ->where("isActive", 1)
            ->get());
        $coop_edited = count(DB::connection("seed_coop_db")->table("tbl_adjustment_logs")
        ->groupBy("coopId")
        ->get());

        // $most_adjusted =  DB::connection("seed_coop_db")->table("tbl_adjustment_logs")
        // ->groupBy("coopId")
        // ->groupBy(DB::raw("DATE_FORMAT(date_created, '%Y-%m-%d')"))
        // ->get();

        $coop_name = DB::connection("seed_coop_db")->table("tbl_adjustment_logs")
                        ->join("tbl_cooperatives", "tbl_cooperatives.coopId", "=", "tbl_adjustment_logs.coopId")
                        ->groupBy("tbl_adjustment_logs.coopId")
                        ->get();

            

        return view("coopDashboard.commitment_logs")
            ->with("coop_count", $coop_count)
            ->with("coop_edited", $coop_edited)
    ...
    public function index(){
        ...
        $seed_variety = DB::table($GLOBALS['season_prefix'].'seed_seed.seed_characteristics')
            ->groupby("variety")
            ->orderBy("variety")
            ->get();
    ...
    public function getCoopID(Request $request){
        return DB::connection('seed_coop_db')->table('tbl_commitment_regional')
    ...
    public function coop_tbl(){
        ...
        ->addColumn('action', function($row){
            ...
            if(count($commitments) > 0){
                ...
                return '
                        <a href="" data-toggle="modal" data-target="#coopDetails" data-id="'.$row->coopId.
                        '" data-name="'.$row->coopName.'" data-bags="'.
                        $bags.
    ...
    public function coopSaveTable(Request $request){
		...
		->addColumn('region', function($row){
            ...
            if($row->coopID == 68)
            {
                return $region = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_regional')->where('commitmentID', $row->id)->value('station');
            }
            else
            {
                            ...
                            if($tbl_regional != null){
                                ...
                                $totals = DB::connection('seed_coop_db')->table('tbl_total_commitment')
                                    ...
                                    if($totals != null){
                                            ...
                                            if($request->coop_id == 68)
                                            {
                                                DB::connection('seed_coop_db')->table('tbl_commitment_regional')
                                                    ->where("id", $tbl_regional->id)
                                                    ->update([
                                                        "region_name" => 'ANY REGION',
                                                        "station" => $request->new_region,
    ...
    public function coop_details_tbl(Request $request){
        return Datatables::of(DB::connection('seed_coop_db')->table('tbl_commitment')
            ->select("tbl_commitment.*")
            ->where('coopID', '=', $request->coopDetailsID)
            ->join("tbl_commitment_regional", "commitmentID","=", "tbl_commitment.id")
            ->orderBy('tbl_commitment_regional.region_name', 'ASC')
            ->orderBy('tbl_commitment_regional.commitmentID', 'ASC')
        )
        ->addColumn('date_add', function($row){
            return date("F j, Y", strtotime($row->date_added)); 
        })
        ...
        ->addColumn('action', function($row){
            // $btn = '<a href="javascript:void(0);" data-id="'.$row->id.'" class="btn btn-danger btn-sm" onclick="removeRecordDetails(this);" ><i class="fa fa-toggle-down"></i> Set as Inactive Variety</a>';
            $btn = "";
        if(Auth::user()->roles->first()->name == "rcef-programmer" || Auth::user()->username == "e.tolentino" || Auth::user()->username == "mcrmercado" ){

           $coop_regional = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_regional')->where('commitmentID', $row->id)->first();
		...
		->addColumn('region', function($row){
            if($row->coopID == 68)
            {
                return DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_regional')->where('commitmentID', $row->id)->value('station');
            }
            else
            {
    ...
    public function updateCoopMOA(Request $request){
        ...
        $escapedCoopId = escapeshellarg($coop_id);
        $command = "$pythonPath \"$scriptPath\" $escapedCoopId";
    ...
    public function updateCoopTargetEfficiency(Request $request){
        DB::connection('seed_coop_db')->table('tbl_cooperatives')
        ->where('coopId', $request->coop_id)
        ->update([
            'target_efficiency' => $request->tar_eff
        ]);
    ...
    public function compute_variety_total($deliveries, $seed_variety) {
        ...
        foreach ($deliveries as $row) {
            ...
            $variety_count_transfers = DB::table($GLOBALS['season_prefix'].'rcep_delivery_inspection.tbl_actual_delivery as ad')
                ->where('ad.remarks', "LIKE", '%' . $row->batchTicketNumber . '%')
                ->where('ad.seedVariety', "=", $seed_variety)
                ->sum('totalBagCount');
            
            // Initialize $total_variety_transfers
            $total_variety_transfers = $variety_count_transfers; 
    
            if ($variety_count_transfers) {
                $total_variety += $variety_count + $total_variety_transfers;
            } else {
    ...
    public function coop_seeds_tbl(Request $request){
        ...
        foreach($commitments as $row){
            $variety_total = $this->compute_variety_total($confirmed_delivery_list, $row->seed_variety);
            $confirmed_total = $this->compute_confirmed_total($row->seed_variety, $request->coop_accre);
     ...
     public function coop_regional_tbl(Request $request){
        $region_list = array();
        $variety_list = array();
        $commitment_list = array();
        $delivered_list = array();
            ...
            $varietyList = DB::connection('seed_coop_db')->table('tbl_commitment_regional')->where('coop_Id', $coop_id)->where('region_name', $commitment_data->region_name)->groupBy("seed_variety")->get();
                foreach ($varietyList as $variety) {
                   ...
                   // dd($region->regio);
                       ...
                       array_push($commitment_list, intval($commitment_value));
                       array_push($delivered_list, $delivered);
...
//        dd($series_arr);
    ...
    */
    ...
    public function coop_rla_edit_tbl(Request $request){
        ...
        ->addColumn('action', function($row){
            $edit_btn = "<a href='".route('rla.monitoring.home', $row->rlaId)."' class='btn btn-warning btn-sm' style='border-radius:20px;'><i class='fa fa-edit'></i> EDIT RLA</a>";
            return $edit_btn;
        })
        ->make(true);
    }
    ...
    public function confirm_edit_rla(Request $request){
		...
		if(count($delivery_data) > 0){
			...
			if($seed_volume <= 200){
					...
					return redirect()->route('rla.monitoring.home');
    ...
    public function coopAdjustmentVarities(Request $request){
        $coop_commitments = DB::table($GLOBALS['season_prefix'].'rcep_seed_cooperatives.tbl_commitment_regional')
            ...
            if(Auth::user()->username=="jpalileo"){
                $editor = "true";
            }else{
                $editor = "false";
            }

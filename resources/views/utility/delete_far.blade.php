            ...
            <div class="x_title">
                ...
                <section class="group_selector">
                        <div class="selectors" id="selectors style="display: inline-block;>
                            ...
                            <div>
                            <button type="button" id='deleteFAR' class="btn btn-warning submit" disabled>Delete Generated FAR</button> 
                            </div>
                        </div>
                </section>
            </div>
...
@push('scripts')
    ...
    <script> 
        ...
        $('#provinces').change(() => {
            ...
            $.ajax({ 
                type: 'POST',
                url: "{{ route('deleteGeneratedFAR.getMuni') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    prov: $prov
                },
        ...
        $('#municipality').on('change', ()=>{
            ...
            if($muni !== 'default')
            {
                $('#deleteFAR').removeAttr('disabled');
            }
            else {
                $('#deleteFAR').attr('disabled', true);
            }
        });

        $('#deleteFAR').on('click', () => {
            var season_prefix = "<?php echo $GLOBALS['season_prefix']; ?>";
            Swal.fire({
                title: 'Are you sure?',
                text: "Delete all generated FARs for the selected Province and Municipality?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        method: 'GET',
                        url: "https://rcef-checker.philrice.gov.ph/api/clearFARs" + "/" +season_prefix+ "/" +prov + "/"+muni,
                        data: {
                            _token: _token,
                        },
                        success: function (data) {
                            if(data.status == 'success')
                            {
                                ...
                                $.ajax({ 
                                    type: 'POST',
                                    url: "{{ route('deleteGeneratedFAR.resetStatus') }}",
                                    data: {
                                        _token: "{{ csrf_token() }}",
                                        prov: prov,
                                        muni: muni

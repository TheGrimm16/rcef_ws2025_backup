...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <h3>Online Encoding for Low Land Holders (< 0.1 ha) </h3>
        </div>
    </div>
            ...
            <div class="card" style="padding:5px;">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input  class='form-control' onkeyup='compute_bags();' onchange='compute_bags();' id='target_area' name='target_area' type="number" value="0" disabled>
                        <label style="color: #888; font-size: 1rem; font-style: italic;" for="bags_computation" id="bags_computation">Equivalent bag(s): 1</label>
                    </div>
                </div>
    ...
    @include("virtual_encoding.select_members_modal")
...
@push('scripts')
    ...
    <script>
        ...
        var lowLandHoldersArray = [];
        var totalAreaElement = 0;
        var remaining_area = 0;
        var province = '';
        var municipality = '';
        var db_ref = '';
        var final_area = '';
        ...
        var is_pwd = '';
        var tel_no = '';
        var table = $('#lowland_tbl').DataTable();
        var globalFinalArea = 0;
        ...
        table.on('draw.dt', function () {
            $('.members').each(function() {
                ...
                lowLandHoldersArray.forEach((element, index) => {
    ...
    $("#new_farmer").on("click", function(){
        ...
        $('#select_members_modal').on('show.bs.modal', function (e) {
            // var db_ref = $(e.relatedTarget).data('db_ref');
            remaining_area = $(e.relatedTarget).data('remaining_area');
            // console.log(remaining_area);    
            // var claiming_prv = $(e.relatedTarget).data('claiming_prv');
            // province = $(e.relatedTarget).data('province');
            // municipality = $(e.relatedTarget).data('municipality');
            
            province = $(e.relatedTarget).data('province');
            municipality = $(e.relatedTarget).data('municipality');
            db_ref = $(e.relatedTarget).data('db_ref');
            final_area = $(e.relatedTarget).data('final_area');
            claiming_prv =  $(e.relatedTarget).data('claiming_prv');
            remaining =  $(e.relatedTarget).data('remaining');
            birthdate =  $(e.relatedTarget).data('birthdate');
            mother_name =  $(e.relatedTarget).data('mother_name');
            is_ip =  $(e.relatedTarget).data('is_ip');
            tribe_name =  $(e.relatedTarget).data('tribe_name');
            is_pwd =  $(e.relatedTarget).data('is_pwd');
            tel_no =  $(e.relatedTarget).data('tel_no');

            $("#lowland_tbl").DataTable().clear();
                        $("#lowland_tbl").DataTable({
                            bDestroy: true,
                            autoWidth: false,
                            ...
                            pageLength: 10,
                            ajax: {
                                url: "{{ route('searchMembers_lowland') }}",
                                dataType: "json",
                                type: "POST",
                                data: {
                                _token: "{{ csrf_token() }}",
                        ...
                        totalAreaElement = document.getElementById("total_area");
                        totalAreaElement.innerHTML = remaining_area;
                        lowLandHoldersArray = [];

        });

        $('#confirmMembers').on('click', function(e){
            e.preventDefault();
            var totalArea = 0;
            ...
            totalArea += parseFloat(remaining_area,4);
            totalArea = parseFloat(totalArea.toFixed(4));
            globalFinalArea = totalArea;

            var combinedParcel = "<div class='col-md-4'> <a class='btn btn-dark btn-lg parcel_btn' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#select_dop_modal'data-db_ref='"+db_ref+"' data-mother_name = '"+mother_name+"' data-is_ip = '"+is_ip+"' data-tribe_name = '"+tribe_name+"' data-is_pwd   = '"+is_pwd+"' data-birthdate  = '"+birthdate+"' data-claiming_prv = '"+claiming_prv+"' data-tel_no = '"+tel_no+"' data-served = 'true' data-remaining='"+remaining+"' data-final_area = '"+totalArea+"' data-remaining_area = '"+remaining_area+"' data-prv='"+province+"' data-municipality='"+municipality+"' data-province='"+province+"' style='width:100%;'>"+province+"<br> "+municipality+"<br>"+totalArea+" available out of "+totalArea+" (ha) </a> </div>";
            
            
            $("#parcelary_list").empty();
            $("#parcelary_list").append(combinedParcel);
            
            $("#select_members_modal").modal('hide');

        });

        $('#lowland_tbl').on('change', '.members', function() {
            // Handle checkbox change event here
            var old = parseFloat($('#total_area').text());
            ...
            if ($(this).prop('checked')) {
                ...
                var indexToRemove = -1;
                lowLandHoldersArray.forEach(function (item, index) {
                    if (item.rsbsa == checkbox.data('rsbsa') && item.dbref == checkbox.data('dbref') && item.area == area) {
                        indexToRemove = index;
                    }
                });
                ...
                if (indexToRemove !== -1) {
                    lowLandHoldersArray.splice(indexToRemove, 1);
                }
            }

            $("#total_area").text(parseFloat(tmp).toFixed(4));

            if(tmp >= 0.1 && tmp <= 0.5){
                $('.fin').css('background-color', 'rgba(68, 243, 68, 0.568)');
                $('.nai').css('background-color', 'rgba(57, 194, 57, 0.568)');
                $('#confirmMembers').removeAttr('disabled');
            }else{
                $('.fin').css('background-color', 'rgba(179, 36, 38, 0.568)');
                $('.nai').css('background-color', 'rgba(179, 57, 59, 0.568)');
                $('#confirmMembers').attr('disabled', true);
            }

            // console.log(lowLandHoldersArray);
        });
        ...
        $('#select_dop_modal_new').on('show.bs.modal', function (e) {
            ...
            $("#mother_last_name").val(mother_name);
    ...
    $("#save_update").on("click", function(){
        // TRAPPER
            var members = JSON.stringify(lowLandHoldersArray);
                            ...
                            success: function(result){
                                ...
                                if(result["status"] != "1"){
                                    ...
                                    if(remaining_new > 0 ){
                                        ...
                                        $("#new_parcel_"+d.float_id).data("data-mother_name", d.mother_name);
    ...
    $("#irrigated").on("click", function(){
        ...
        $("#dop_select").on("click", function(){
               ...
               $("#dop_selected_vs").val(select_dop);
               $("#dop_selected_name").val(dop_selected_name);
               $("#target_area").val(globalFinalArea);
        });
        ...
        $('#select_dop_modal').on('show.bs.modal', function (e) {
             ...
             var mother_name =  $(e.relatedTarget).data('mother_name');
        ...
        function get_parcel_list(db_ref, prv){
            ...
            success: function(result){
                ...
                $.each(result["parcel_list"], function (i, d) {
                    ...
                    if(d.remaining > 0 ){
                        var parcel = "<div class='col-md-4'> <a id='parcel_"+d.id+"' data-toggle='modal' data-backdrop='static' data-keyboard='false' data-target='#select_members_modal' data-db_ref='"+d.id+"' data-mother_name = '"+d.mother_name+"' data-is_ip = '"+d.is_ip+"' data-tribe_name = '"+d.tribe_name+"' data-is_pwd   = '"+d.is_pwd+"' data-birthdate  = '"+d.birthdate+"' data-claiming_prv = '"+d.claiming_prv+"' data-tel_no = '"+d.tel_no+"' data-served = 'true' data-remaining='"+d.remaining+"' data-final_area = '"+d.final_area+"' data-remaining_area = '"+d.remaining_area+"' data-prv='"+d.prv+"' data-municipality='"+d.municipality+"' data-province='"+d.province+"' class='btn btn-dark btn-lg parcel_btn' style='width:100%;'  value='"+d.prv+";"+d.id+"'  >"+d.province+"<br> "+d.municipality+"<br>"+d.remaining_area+" available out of "+d.final_area+" (ha) </a> </div>";
                    }else{
        ...
        function search(){
            ...
            var search_bar = $("#search_bar").val();
                ...
                $('#dataTBL').DataTable({
                    ...
                    "pageLength": 25,
                    "ajax": {
                        "url": "{{route('virtual_search_lowland')}}",
                        "dataType": "json",
                        "type": "POST",
                        "data":{
                            "_token": "{{ csrf_token() }}",

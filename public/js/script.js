...
$(document).ready(function () {
    ...
    // jayvee dela cruz: error removed GET http://dbmp2.philrice.gov.ph/rcef_ws2022/coop_summary_total 401 (Unauthorized)
    // $.ajax({
...
function confirm_variety(_token, farmer_id, bags, batch_ticket_no, variety) {
...
$('#distribution_municipality').on('change', function () {
    ...
    $.ajax({
        ...
        success: function (source) {
            let options = "<option></option>"
            source.forEach(function (item) {
                options += "<option value='" + item['dropOffPoint'] + "'>" + item['dropOffPoint'] + "</option>"
            })
...
$('#save_farmer').on('click', function () {
    ...
    let farm_area = $('#add_farmer_modal #farm_area2').val()
    ...
    if (document.getElementById('on_list').checked == true) {
        if (rsbsa_control_no == "" || farm_area == "" || qr_code == "" || variety == undefined || sex == 0 || firstname == "" || lastname == "") {
